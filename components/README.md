...
flowchart TB
    ...
    subgraph Wrapper[Корневой контейнер страницы]
        Header[Header]
        Main[<main> — flex-колонка секций]
        Footer[Footer]
    end
    ...
    subgraph MainFlow[Секции внутри <main> (по порядку в app/page.tsx)]
        Hero[Hero (Section id="hero", variant="dark")]
        Intro[Intro (Section, мобильный текст)]
        Bridge[DetToDetaiBridge (Section, связь DET ↔ DETai)]
        Split[DetDetaiSplit (Section, выбор направления)]
        Audience[AudienceSection (Section, карточки аудиторий)]
        Fundament[FundamentDetSection (Section id="fundament-det")]
        Mission[Mission (Section id="mission", variant="dark")]
    end

    %% Детализация Hero
    Hero --> HeroContent[Колонка текста: H1, подзаголовок, два абзаца]
    Hero --> HeroCTA[CTA: Button → #fundament-det, Button → #mission]
